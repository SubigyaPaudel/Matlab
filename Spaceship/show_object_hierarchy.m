...
drawnow;
%putting translation matrix on the for loop to obtain continuous
%transformation

yaw = pi;
roll = -pi/4;
for i = 1:100
   trf_rotate = makehgtform('zrotate', yaw);
   trf_rotate2 = makehgtform('xrotate', roll);
   trf_translate2 = makehgtform('translate', [i * (-2) i i/2]);
   trf_combined = trf_rotate2*trf_translate2;
   set(trf_ship2_ship1, 'Matrix', trf_combined);
   set(trf_ship1_root, 'Matrix', trf_rotate);
   drawnow;
end
